...
static void select_folder_draw(TWidget * wid, ttk_surface srf)
{
	...
	if (ttk_screen->w < 160) {
		ti_multiline_text_softdep(srf, ttk_textfont, wid->x, wid->y, wid->w, wid->h, ttk_ap_getx("window.fg")->color,
			_("Type the videos folder location."), -1, 0, 0, 0, 0);
	} else if (ttk_screen->w < 200) {
		ti_multiline_text_softdep(srf, ttk_textfont, wid->x, wid->y, wid->w, wid->h, ttk_ap_getx("window.fg")->color,
			_("Type the folder where the videos are located."), -1, 0, 0, 0, 0);
	} else {
		ti_multiline_text_softdep(srf, ttk_textfont, wid->x, wid->y, wid->w, wid->h, ttk_ap_getx("window.fg")->color,
			_("Type the folder where the videos are located. To be able tu use videos selector in the main menu."), -1, 0, 0, 0, 0);
	}
}
...
static TWindow *new_folder_window()
{
	TWindow * ret;
	TWidget * wid;
	TWidget * wid2;
	ret = pz_new_window(_("Videos"), PZ_WINDOW_NORMAL);
...
static int is_mvpd(const char *file)
{
        return check_ext(file, ".mvpd");
}

static TWindow *load_file(char *file)
{
	char command[60];
	sprintf(command, "%s %s", pz_module_get_datapath (module, "Mv-Player"), file);
	const char *const argv[] = {"sh", "-c", command, NULL};
	pz_execv("/bin/sh", (char *const *)argv);
	return TTK_MENU_DONOTHING;
}

static TWindow *load_file_handler(ttk_menu_item *item)
{
	return load_file(item->data);
}

static PzWindow *new_browser_ext_window()
{
        struct stat st;  
	char folder[30];
	sprintf(folder, "%s", pz_get_string_setting(config, FOLDER));
        ...
        if (!pz_get_setting(config, FOLDER)) {
          ...
          pz_warning(_("Select a folder in /Setting/Select folder/Videos"));
          return TTK_MENU_DONOTHING;
        }
        ...
        return open_directory_ext_softdep(folder, "Videos", is_mvpd); 
}
...
static void cleanup()
{
	...
	config = 0;
	pz_browser_remove_handler(is_mvpd);
}
...
static void init_launch() 
{
	...
	binary = pz_module_get_datapath(module,"Mv-Player");

        if (!stat(binary, &st) == S_IXUSR || 00100)
            chmod(binary, S_IRWXU);
	
        browser_extension.name = N_("Play Video");
	browser_extension.makesub = load_file_handler;
	pz_browser_add_action (is_mvpd, &browser_extension);
	pz_browser_set_handler(is_mvpd, load_file);
	
	if (browser_available()) 
	  pz_menu_add_action_group("/Videos", "Media", new_browser_ext_window);
        else pz_warning(_("Module browser is missing"));

        if (tiwidgets_available())
	pz_menu_add_action_group("/Setting/Select Folder/Videos", "Media", new_folder_window);
